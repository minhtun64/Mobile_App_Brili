...
export default function M_ChatListScreen({ navigation }) {
    ...
    const fetchDataFromFirebase = async () => {
        ...
        myUserIdNo = userSnapshot.val().id;
        ...
        usersArr[myUserIdNo] = newUser;
    
        onValue(reference, async (snapshot) => {
            const data = snapshot.val();
            const filteredMessages = Object.values(data).filter(
                (message) => message.sender_id === myUserIdNo || message.receiver_id === myUserIdNo
            );
            ...
            try {
                ...
                for (const message of filteredMessages) {
                    ...
                    const userID = parseInt(participants.replace(new RegExp(`_|${myUserIdNo}`, 'g'), ''));
                    if (!userIDs.includes(userID)) {
                        userIDs.push(userID);
                        const userRef = ref(database, 'user/');
                        const queryRef = query(userRef, orderByChild("id"));
                        const promise = get(queryRef).then((snapshot) => {
                            snapshot.forEach((childSnapshot) => {
                                const userKey = childSnapshot.key; // Lấy key của người dùng
                                const userData = childSnapshot.val(); // Lấy dữ liệu người dùng
    
                                if (userData.id == userID) {
                                    const senderName = userData && userData.name ? userData.name : '';
                                    const senderAvatar = userData && userData.avatar ? userData.avatar : '';
                                    const newUser = {
                                        id: userID,
                                        name: senderName,
                                        avatar: senderAvatar,
                        ...
                        getUserPromises.push(promise);
                    }
                }
    
                // Wait for all the getUserPromises to complete
                ...
                <View style={[styles.titleRow, styles.row]}>
                    <View style={styles.userAvatarContainer}>
                        <Image
                            ...
                            // source={{ uri: usersData[myUserIdNo].avatar }}
                        />
                    </View>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() =>
                                navigation.navigate('M_Chat', {
                                    propId: usersData[item.sender_id !== myUserIdNo ? item.sender_id : item.receiver_id]?.id,
                                    propName: usersData[item.sender_id !== myUserIdNo ? item.sender_id : item.receiver_id]?.name,
                                    // propAvatar: usersData[item.sender_id !== myUserIdNo ? item.sender_id : item.receiver_id]?.avatar
                                })
                            }
                        >
                            <View style={[styles.wrapping, styles.row]}>
                                <View style={styles.userImgWrapper}>
                                    {/* <Image style={styles.userImg} source={{ uri: usersData[item.sender_id !== myUserIdNo ? item.sender_id : item.receiver_id]?.avatar }} /> */}
                                </View>
                                <View style={styles.textSection}>
                                    <View style={styles.userInfoText}>
                                        <Text style={styles.userName}>{usersData[item.sender_id !== myUserIdNo ? item.sender_id : item.receiver_id]?.name}</Text>
